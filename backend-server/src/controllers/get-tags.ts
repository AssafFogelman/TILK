    ...
    try {
        const result = await db
            .select({
                ...
                tags: sql<{ tagContent: string }[]>`json_agg(json_build_object('tagContent', ${tags.tagContent}))`,
            })
            .from(tagCategories)
            .leftJoin(tagsTagCats, eq(tagCategories.tagCategoryId, tagsTagCats.tagCategoryId))
