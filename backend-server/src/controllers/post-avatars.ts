...
export const postAvatars = async (c: Context) => {
    try {
        ...
        console.log("Received data:", avatars.map((avatar: AvatarType) => JSON.stringify(avatar).substring(0, 100) + '...')); // Log a preview of the received data
        ...
        const originalAvatarTempPaths = (await Promise.all(avatars.map(async (avatar: AvatarType, index: number) => {
            ...
            if (typeof avatar === "string") {
                ...
                const serverFilePath = process.cwd() + "/public" + avatar.split("public")[1];
        ...
        const smallAvatarFullPath = path.join(smallAvatarDirectory, smallAvatarName);
        ...
        const finalAvatarPaths = await Promise.all(originalAvatarTempPaths.map(async (filePath) => {
        ...
        const dbPathsOriginal = finalAvatarPaths.map((path) => ("public" + path.split("public")[1]).split("\\").join("/"));
        const dbPathsSmall = ("public" + smallAvatarPath.split("public")[1]).split("\\").join("/");
        ...
        return c.json({originalSize: [...dbPathsOriginal], smallSize: dbPathsSmall}, 200);
    } catch (error) {
