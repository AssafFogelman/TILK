...
const SelectAvatarScreen = () => {
    ...
    useEffect(() => {
        (async () => {
            try {
                let {originalAvatars} = await axios.get("/user/avatar-links").then(res => res.data);

                originalAvatars = originalAvatars.map((avatarPath: string) => process.env.EXPO_PUBLIC_SERVER_ADDRESS + "/" + avatarPath);
                console.log("originalAvatars:", originalAvatars);
                if (originalAvatars.length > 0) {
                    setMainAvatar(originalAvatars[0])
                }
                if (originalAvatars.length > 1) {
                    setSmallAvatars([...originalAvatars.slice(1, 4), ...Array(4 - originalAvatars.length).fill(null)]);
                }
            } catch (error) {
    ...
    const handleNext = async () => {
        ...
        try {
            ...
            const base64Files = await Promise.all(allAvatarUris.map(async (uri) => {
                if (!uri) return null;
                if (uri.startsWith("http")) return uri;
                if (uri.startsWith("file")) return await FileSystem.readAsStringAsync(uri, {encoding: FileSystem.EncodingType.Base64});
