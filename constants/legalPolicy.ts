import { I18nManager } from "react-native";

export const legalPolicy = `
Privacy Policy
Effective Date: ${getDate(new Date())}
Last Updated: ${getDate(new Date())}



1. Introduction
Elobby Inc. (“we,” “us”) operates the TILK app (“App”). This Privacy Policy explains how we collect, use, and protect your information. By using the App, you consent to this policy.

2. Data Collected

Personal Data: User ID, hobbies, and location (collected via GPS or IP address).

Non-Personal Data: Device type, OS version, and usage patterns.

3. How We Use Data

Match users with shared hobbies in proximity.

Improve App functionality and user experience.

Retain data for 6 months after account inactivity, after which it is deleted.

4. Data Sharing

With Other Users: Hobbies and approximate location (e.g., city) are visible to nearby users.

Third Parties: Only for legal compliance or safety reasons (e.g., law enforcement).

5. Your Rights

Access, correct, or delete your data via the App settings.

Withdraw consent by deleting your account.

GDPR/CCPA rights: Contact us at assaf.fogelman@gmail.com.

6. Security
Data is encrypted in transit and at rest. No method is 100% secure, but we use industry-standard practices.

7. Children’s Privacy
The App is not intended for users under 13. We do not knowingly collect data from children (see Children’s Privacy section).

8. Changes to Policy
Updates will be posted in the App. Continued use constitutes acceptance.

Contact: assaf.fogelman@gmail.com

Terms and Conditions
1. Acceptance
By using the App, you agree to these Terms.

2. Eligibility

Users must be at least 13 years old.

3. Account Responsibility

Keep your credentials secure.

Notify us of unauthorized use.

4. Prohibited Conduct

Illegal activities, harassment, or data scraping.

Reverse engineering or disrupting the App.

5. Termination
We may suspend accounts for violations. Users may delete accounts at any time.

6. Disclaimers
The App is provided “as is.” We disclaim warranties for accuracy or uptime.

7. Limitation of Liability
We are not liable for indirect damages arising from App use.

8. Governing Law
Israel law applies. Disputes resolved in Israel.

Contact: assaf.fogelman@gmail.com

End-User License Agreement (EULA)
1. License Grant
Elobby Inc. grants a non-exclusive, revocable license to use the App for personal purposes.


2. Restrictions

No modification, redistribution, or commercial use.

Compliance with app store terms (Apple App Store/Google Play).

3. Updates
Automatic updates may occur. Continued use implies acceptance.

4. Termination
Breach of terms results in license termination.

5. Warranty
No warranties provided. Use at your own risk.

Cookie Policy
The App uses one session cookie (userId) for authentication. No third-party or tracking cookies are used. Data is deleted after 6 months of inactivity.

Copyright Notice
© ${new Date().getFullYear()} Elobby Inc. All rights reserved. Unauthorized copying, distribution, or use of App content is prohibited. Report infringement to assaf.fogelman@gmail.com.


Children’s Privacy
The App complies with COPPA and does not target users under 13.

If we discover underage users, their data will be deleted promptly.

Intellectual Property Rights
The App’s code, design, and content are owned by Elobby Inc.

Users retain rights to their hobby data but grant us a license to process it for App functionality.

App Store Compliance
Google Play: Complies with Developer Policy.

Apple App Store: Follows App Store Guidelines.

Users must comply with each store’s terms.
`;

function getDate(dateAsDate: Date) {
  const day = dateAsDate.getDate();
  const month = dateAsDate.getMonth() + 1; // Months are 0-indexed
  if (I18nManager.isRTL) {
    return `${day.toString().padStart(2, "0")}.${month
      .toString()
      .padStart(2, "0")}.${dateAsDate.getFullYear()}`;
  }

  return `${month
    .toString()
    .padStart(
      2,
      "0"
    )}.${day.toString().padStart(2, "0")}.${dateAsDate.getFullYear()}`;
}
